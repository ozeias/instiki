...
class WikiController < ApplicationController
  ...
  def rss_with_content
    if rss_with_content_allowed?
      ...
      render(:text => 'RSS feed with content for this web is blocked for security reasons. The web is password-protected and not published', :status => 403)
    end
  end
  ...
  def show
    if @page
      begin
        ...
        @show_diff = (params[:mode] == 'diff')
        render :action => 'page'
      # TODO this rescue should differentiate between errors due to fing and errors in 
